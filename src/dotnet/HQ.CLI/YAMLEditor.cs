using CsvHelper;
using FluentResults;
using Microsoft.Extensions.FileProviders;
using System;
using System.Collections.Generic;
using System.Diagnostics;
using System.IO;
using System.Linq;
using System.Reflection;
using System.Text;
using System.Threading.Tasks;
using YamlDotNet.Serialization.NamingConventions;
using YamlDotNet.Serialization;
using YamlDotNet.Core;

namespace HQ.CLI
{
    internal class YAMLEditor<T>
    {
        private T _source;
        private string _sourceYAML;
        private bool _preserveTempFile = false;
        private readonly string _tempFilePath;
        private readonly string _defaultEditor;
        private readonly Func<T, Task<ResultBase>> _validator;
        private readonly ISerializer _serializer;
        private readonly IDeserializer _deserializer;

        public YAMLEditor(T source, Func<T, Task<ResultBase>> validator)
        {
            _serializer = new SerializerBuilder()
                .WithNamingConvention(CamelCaseNamingConvention.Instance)
                .Build();

            _deserializer = new DeserializerBuilder()
                .WithNamingConvention(CamelCaseNamingConvention.Instance)
                .Build();

            _source = source;
            _sourceYAML = _serializer.Serialize(source);

            _validator = validator;

            _tempFilePath = Path.Join(Path.GetTempPath(), $"hq-edit-{DateTime.Now.ToString("yyyyMMddTHHmmss")}.yaml");

            _defaultEditor = "vi";
            if (OperatingSystem.IsWindows())
            {
                _defaultEditor = "notepad";
            }
        }

        private async Task WriteTempFile(ResultBase? result = null)
        {
            var yaml = _serializer.Serialize(_source);
            var file = new StringBuilder();

            file.AppendLine("# Make changes below. Comments are ignored.");
            file.AppendLine("# If an error occurs when saving, this file will be reopened.");
            
            if(result != null && result.IsFailed)
            {
                file.AppendLine("# Errors:");
                foreach(var error in result.Errors)
                {
                    file.AppendLine("## - " + error.Message);
                }
            }

            file.AppendLine("##");
            file.AppendLine(yaml);

            await File.WriteAllTextAsync(_tempFilePath, file.ToString());
        }

        public async Task<int> Launch()
        {
            try
            {
                var valid = false;
                var parsed = _source;
                var yaml = _serializer.Serialize(parsed);
                await WriteTempFile();

                do
                {
                    try
                    {
                        var process = new Process();
                        process.StartInfo.FileName = _defaultEditor;
                        process.StartInfo.UseShellExecute = true;
                        process.StartInfo.Arguments = _tempFilePath;
                        process.Start();
                        process.WaitForExit();

                        var contents = await File.ReadAllTextAsync(_tempFilePath);
                        var source = _deserializer.Deserialize<T>(contents);
                        if(source == null)
                        {
                            Console.WriteLine("Unable to deserialize.");
                            return 1;
                        }

                        var sourceYAML = _serializer.Serialize(source);
                        if (_sourceYAML.Equals(sourceYAML))
                        {
                            Console.WriteLine("No changed found, exiting.");
                            return 0;
                        }

                        _source = source;
                        _sourceYAML = sourceYAML;

                        var result = await _validator(_source);
                        valid = result.IsSuccess;

                        if(!valid)
                        {
                            await WriteTempFile(result);
                        }
                    }
                    catch (YamlException ex)
                    {
                        Console.WriteLine(ex.Message);
                        Console.WriteLine();
                        Console.WriteLine("Error parsing file, temporary file is available at {0}", _tempFilePath);
                        _preserveTempFile = true;
                        break;
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine(ex.ToString());
                    }
                } while (!valid);
            }
            finally
            {
                if(!_preserveTempFile)
                {
                    File.Delete(_tempFilePath);
                }
            }

            return 0;
        }
    }
}
